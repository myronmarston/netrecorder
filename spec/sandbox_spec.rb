...
describe NetRecorder::Sandbox do
  ...
  describe 'on creation' do
    it "should raise an error if given an invalid record mode" do
      lambda { NetRecorder::Sandbox.new(:test, :record => :not_a_record_mode) }.should raise_error(ArgumentError)
    end

    NetRecorder::Sandbox::VALID_RECORD_MODES.each do |mode|
      it "should default the record mode to #{mode} when NetRecorder::Config.default_sandbox_record_mode is #{mode}" do
        NetRecorder::Config.default_sandbox_record_mode = mode
        sandbox = NetRecorder::Sandbox.new(:test)
  ...
  describe '#destroy!' do
    ...
    it "should write the recorded responses a subdirectory if the sandbox name includes a directory" do
      recorded_responses = [NetRecorder::RecordedResponse.new(:get,  'http://example.com', :get_example_dot_come_response)]
      sandbox = NetRecorder::Sandbox.new('subdirectory/test_sandbox')
      sandbox.stub!(:recorded_responses).and_return(recorded_responses)
    ...
    it "should write both old and new recorded responses to disk" do
      ...
      sandbox = NetRecorder::Sandbox.new('previously_recorded_responses')
      sandbox.should have(2).recorded_responses
      new_recorded_response = NetRecorder::RecordedResponse.new(:get, 'http://example.com/bar', :example_dot_com_bar_response)
  ...
  describe '#destroy for a sandbox with previously recorded responses' do
    ...
    it "should not re-write to disk the previously recorded resposes if there are no new ones" do
      NetRecorder::Config.cache_dir = File.expand_path(File.dirname(__FILE__) + '/fixtures/sandbox_spec')
      yaml_file = File.join(NetRecorder::Config.cache_dir, 'example.yml')
      sandbox = NetRecorder::Sandbox.new('example', :record => :none)
